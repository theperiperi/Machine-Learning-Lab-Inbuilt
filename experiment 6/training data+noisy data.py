...
num_noisy_samples = 100  # Number of noisy samples to add
noise_level = 0.1  # Level of noise to add


noisy_X = X_train + np.random.normal(loc=0, scale=noise_level, size=X_train.shape)
noisy_y = y_train  # No change in target labels


# Concatenate noisy data with original training data
X_train_augmented = np.vstack((X_train, noisy_X))
y_train_augmented = np.vstack((y_train, noisy_y))
